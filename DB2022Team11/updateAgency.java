	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == Show_Button) {
			...
			for(int i=0; i<Head.size(); i++) {
				if(Head.get(i) == "agency_id") {
					ID_COLUMN = i;
				}
				else if(Head.get(i) == "agency_name") {
					NAME_COLUMN = i;
				} else if(Head.get(i) == "agency_number") {
					NUMBER_COLUMN = i;
				}
			}
			...
			try {
				...
				while(r.next()) {
					...
					input[2] = r.getString("agency_name");
					input[3] = r.getString("agency_address");
					input[4] = r.getString("agency_number");
					model.addRow(input);
				}
		...
		if(e.getSource() == Update_Button) {
			...
			try {
				String columnName_1 = model.getColumnName(2);  // 부동산 이름
				String columnName_2 = model.getColumnName(4);  // 전화번호
				if(columnName_1 == "agency_name" && columnName_2 == "agency_number") {
